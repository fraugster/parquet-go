...
func BenchmarkCompressorMap(b *testing.B) {
	block := []byte(`lorem ipsum dolor sit amet, consectetur adipiscing elit, 
sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. 
Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut 
aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in 
...
occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.`)
	...
	for i := 0; i < b.N; i++ {
		for _, m := range methods {
			b, _ := compressBlock(block, m)
			_, _ = decompressBlock(b, m)
		}
	}
...
occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.`)
	...
	for i := 0; i < b.N; i++ {
		for _, m := range methods {
			b, _ := m.CompressBlock(block)
			_, _ = m.DecompressBlock(b)
		}
	}
