...
func metaFile(w io.Writer, address string) error {
	...
	s := reader.GetSchemaDefinition()
	writer := tabwriter.NewWriter(w, 8, 8, 0, '\t', 0)
	printFlatSchema(writer, s, 0)
	return writer.Flush()
}

func printFlatSchema(w io.Writer, s *goparquet.SchemaDefinition, lvl int) {
	dot := strings.Repeat(".", lvl)
	s.Children(
		func(column goparquet.Column) {
			...
			printFlatSchema(w, s.SubSchema(column.Name()), lvl+1)
		})
}
